...
// /ask is the only paid public workflow.  Never permit an environment typo or
// upstream payload to raise its x402 ceiling above the certified demo limit.
const configuredMaxPayment = Number(process.env.TELEGRAPH_MAX_PAYMENT_USDC ?? "0.010000");
const maxPayment = Number.isFinite(configuredMaxPayment) && configuredMaxPayment > 0
  ? Math.min(configuredMaxPayment, 0.01)
  : 0;
