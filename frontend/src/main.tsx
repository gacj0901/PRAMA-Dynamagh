...
function ActivityScope({ aggregate, users }: { aggregate?: ActivityAggregate; users?: boolean }) {
  return <>{users && <Signal label="MANUAL USERS" value={aggregate?.real_users} />}<Signal label="WORKFLOWS" value={aggregate ? `${aggregate.workflows_completed}/${aggregate.workflows_started} complete` : null} /><Signal label="MULTI-INTENT" value={aggregate?.multi_intent_workflows} /><Signal label="TELEGRAPH" value={aggregate?.telegraph_successful_calls} /><Signal label="MINERS" value={aggregate?.real_miners_used.length} /><Signal label="INTENTS" value={aggregate?.intents_used.join(", ") || null} /></>;
}
...
function LiveConsole({ snapshot, refreshedAt }: { snapshot: Snapshot; refreshedAt: string | null }) {
  ...
  const totalWorkflows = (manual?.workflows_started ?? 0) + (m2m?.workflows_started ?? 0) + (autonomous?.workflows_started ?? 0);
  const totalTelegraph = (manual?.telegraph_calls ?? 0) + (m2m?.telegraph_calls ?? 0) + (autonomous?.telegraph_calls ?? 0);
  const totalSettled = (Number(manual?.public_spend_usdc ?? 0) + Number(m2m?.public_spend_usdc ?? 0) + Number(autonomous?.public_spend_usdc ?? 0)).toFixed(6);
  const metric = (label: string, value: string | number | null | undefined) => <Signal label={label} value={value} />;
  return <section className="panel live-console production-activity"><PanelHeading eyebrow="PRODUCTION ACTIVITY" badge={snapshot.apiUp ? "REAL RECORDS · LIVE" : "OFFLINE"} /><div className="production-activity-group"><div className="activity-scope-heading">MANUAL USE</div>{metric("MANDATES", manual?.workflows_started)}{metric("TELEGRAPH CALLS", manual?.telegraph_calls)}{metric("COMPLETED WORKFLOWS", manual ? `${manual.workflows_completed}/${manual.workflows_started}` : null)}</div><div className="production-activity-group"><div className="activity-scope-heading">M2M USE</div>{metric("MANDATES", m2m?.workflows_started)}{metric("TELEGRAPH CALLS", m2m?.telegraph_calls)}{metric("COMPLETED WORKFLOWS", m2m ? `${m2m.workflows_completed}/${m2m.workflows_started}` : null)}</div><div className="production-activity-group"><div className="activity-scope-heading">AUTONOMOUS ACTIVITY</div>{metric("AUTONOMOUS RUNS", autonomousRuns)}{metric("TELEGRAPH CALLS", autonomous?.telegraph_calls)}{metric("COMPLETED WORKFLOWS", autonomous ? `${autonomous.workflows_completed}/${autonomous.workflows_started}` : null)}{metric("SETTLED USDC", autonomous?.public_spend_usdc)}</div><div className="production-activity-group"><div className="activity-scope-heading">TOTAL PRODUCTION</div>{metric("TOTAL WORKFLOWS", totalWorkflows)}{metric("TELEGRAPH CALLS", totalTelegraph)}{metric("SETTLED USDC", totalSettled)}</div><p className="log-caption">Persisted production records, refreshed every 10 seconds. Total production includes Manual, M2M and Autonomous origins.</p>{refreshedAt && <p className="log-caption">REFRESHED {displayDate(refreshedAt)}</p>}</section>;
}
