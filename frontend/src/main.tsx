...
const M2M_CONTRACT = {
  ...
  accepts: ["intent", "request"],
  returns: ["evidence", "evaluation", "decision", "ticket"],
  designed_for: "autonomous_agents",
} as const;

function M2MAppendix() {
  const [open, setOpen] = useState(false);
  return <div className="m2m-appendix">
    <button type="button" className={`m2m-appendix-trigger${open ? " active" : ""}`} aria-expanded={open} aria-controls="m2m-contract-panel" onClick={() => setOpen((current) => !current)}>
      <span>01010111 01000101 01001100 01000011 01001111 01001101 01000101</span> M2M
    </button>
    {open && <section id="m2m-contract-panel" className="m2m-contract-panel" role="dialog" aria-label="M2M integration contract">
      <button type="button" className="m2m-contract-close" aria-label="Close M2M contract" onClick={() => setOpen(false)}>×</button>
      <div className="m2m-contract-heading"><span>M2M APPENDIX</span><strong>autonomous agent interface</strong></div>
      <pre>{JSON.stringify(M2M_CONTRACT, null, 2)}</pre>
    </section>}
  </div>;
}
