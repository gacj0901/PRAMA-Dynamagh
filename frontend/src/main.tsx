...
function ActivityScope({ aggregate, users }: { aggregate?: ActivityAggregate; users?: boolean }) {
  return <>{users && <Signal label="USERS" value={aggregate?.real_users} />}<Signal label="WORKFLOWS" value={aggregate ? `${aggregate.workflows_completed}/${aggregate.workflows_started} complete` : null} /><Signal label="MULTI-INTENT" value={aggregate?.multi_intent_workflows} /><Signal label="TELEGRAPH" value={aggregate?.telegraph_successful_calls} /><Signal label="MINERS" value={aggregate?.real_miners_used.length} /><Signal label="INTENTS" value={aggregate?.intents_used.join(", ") || null} /></>;
}
...
function LedgerRow({ label, value, indent = false }: { label: string; value: number; indent?: boolean }) {
  return <div className={`ledger-row${indent ? " ledger-row-indent" : ""}`}><span>{indent ? "├─ " : ""}{label}</span><strong>{value.toLocaleString("en-US")}</strong></div>;
}
...
function LiveConsole({ snapshot, refreshedAt }: { snapshot: Snapshot; refreshedAt: string | null }) {
  ...
  const autonomous = activity?.autonomous;
  // The console shows live, real activity: manual/M2M plus the autonomous agent's
  // own persisted records. Each line stays traceable to its origin.
  const line = (label: string, value: string | number | null | undefined) => <div className="console-line"><span>{label.padEnd(22, " ")}</span><b>{value === null || value === undefined || value === "" ? "—" : value}</b></div>;
  const publicWorkflows = activity ? `${activity.workflows_completed}/${activity.workflows_started}` : null;
  const agentWorkflows = autonomous ? `${autonomous.workflows_completed}/${autonomous.workflows_started}` : null;
  const totalWorkflowsCompleted = (activity?.workflows_completed ?? 0) + (autonomous?.workflows_completed ?? 0);
  const totalWorkflowsStarted = (activity?.workflows_started ?? 0) + (autonomous?.workflows_started ?? 0);
  const totalTelegraph = (activity?.telegraph_successful_calls ?? 0) + (autonomous?.telegraph_successful_calls ?? 0);
  const allMiners = new Set([...(activity?.real_miners_used ?? []), ...(autonomous?.real_miners_used ?? [])]);
  const allIntents = new Set([...(activity?.intents_used ?? []), ...(autonomous?.intents_used ?? [])]);
  const totalSettled = (Number(activity?.public_spend_usdc ?? 0) + Number(autonomous?.public_spend_usdc ?? 0)).toFixed(6);
  const multiIntentTotal = (activity?.multi_intent_workflows ?? 0) + (autonomous?.multi_intent_workflows ?? 0);
  return <section className="panel live-console"><PanelHeading eyebrow="LIVE ACTIVITY CONSOLE" badge="READ ONLY" /><div className="console-screen" role="status" aria-live="polite"><div className="console-line console-prompt"><span>PRAMA-DYNAMAGH&gt; activity --all</span><b>{snapshot.apiUp ? "OK" : "OFFLINE"}</b></div>{line("API", snapshot.apiUp ? "UP" : "UNAVAILABLE")}{line("AUTHORITY_MODE", snapshot.autonomy ? "BINDING" : null)}{line("POLICY", snapshot.autonomy?.active_policy_count ? "ACTIVE" : "IDLE")}{line("ACTIVE_RUNS", snapshot.autonomy?.active_run_count)}{line("NEXT_DUE", dueLabel(snapshot.autonomy?.next_due_at))}{line("REAL_USERS", activity?.real_users)}{line("WORKFLOWS_PUBLIC", publicWorkflows ? `${publicWorkflows} complete` : null)}{line("WORKFLOWS_AGENT", agentWorkflows ? `${agentWorkflows} complete` : null)}{line("WORKFLOWS_TOTAL", `${totalWorkflowsCompleted}/${totalWorkflowsStarted} complete`)}{line("MULTI_INTENT", multiIntentTotal)}{line("TELEGRAPH_CALLS", totalTelegraph)}{line("MINERS", allMiners.size)}{line("INTENTS", Array.from(allIntents).join(", "))}{line("SETTLED_USDC", totalSettled)}{line("REFRESHED", refreshedAt ? displayDate(refreshedAt) : null)}</div><p className="log-caption">Live read surface. Public and autonomous aggregates come from distinct persisted origins; no action is executed here.</p></section>;
}
