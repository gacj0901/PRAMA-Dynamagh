...
def _facts(
    *,
    ...
    recovery_events = sorted({item.event_type for item in events if "COMPLET" in item.event_type or "RECOVER" in item.event_type or item.event_type in {"G13_RECOVERY_PROBE_STARTED", "ACQUISITION_PAYMENT_RECONCILED"} or (item.event_type == "TELEGRAPH_REQUEST" and (item.metadata_ or {}).get("recovery_probe_authorized") is True)})
