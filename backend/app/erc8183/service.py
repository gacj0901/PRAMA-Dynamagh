...
def request_fixture(session: Session) -> tuple[ERC8183Job, str]:
    existing = session.query(ERC8183Job).filter(
        ...
        ERC8183Job.callback_address == ZERO,
    ...
    job = ERC8183Job(chain_id=CHAIN_ID, diamond_address=DIAMOND.lower(), intent_name=INTENT_NAME, intent_id=INTENT_ID, callback_address=ZERO, params_payload=PARAMS, state="PREPARING")
